                ...
                <div class="main-wrapper">
                    <div class="row">
                      <div class="col">
                        <div class="card">
                          <div class="card-body">
                              <h5 class="card-title">test_join</h5>
                              <p>請輸入您需查詢的項目。</p>
                              <form action="" method="post" class="row g-3 needs-validation" novalidate>

								                <!-- 訂單全域搜尋（輸入編號、名稱） -->
                                <div class="col-md-6">
                                  <label for="all_customer_label" class="form-label">請輸入顧客編號，以合併查詢詳細資料(若要查詢全部請輸入 *) </label>
                      ...
                      <?php
                          if(isset($_POST["customer_number"])){
                            ...
                            if ($customer_number != ""){
                              ...
                              $sql = "SELECT Customer.customer_number, Trade.order_number, Customer.name, Customer.phone_number, Customer.address, Trade.order_time FROM `Trade` inner join `Customer` on Customer.customer_number = Trade.customer_number where Customer.customer_number = '$customer_number'";
                              if ($customer_number == "*"){
                                        $sql = "SELECT Customer.customer_number, Trade.order_number, Customer.name, Customer.phone_number, Customer.address, Trade.order_time FROM `Trade` inner join `Customer` on Customer.customer_number = Trade.customer_number order by Customer.customer_number ASC";
                              }
                            ...
                            <div class="card-body">
                                ...
                                <table id="zero-conf" class="display" style="width:100%">
                                    <thead>
                                      <!--欄位名稱-->
                                                          <tr>
                                                              <th>顧客編號 Customer Number</th>
                                                              <th>訂單編號 Order Number</th>
                                                              <th>顧客名稱 Name</th>
                                                              <th>顧客電話 Phone</th>
                                                              <th>顧客地址 Address</th>
                                                              <th>訂單時間 Order time</th>
                                                          </tr>
                                                      </thead>
                                                      <tbody>
                                                        <?php
